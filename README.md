...
Although in our final model 3 we arrive at a final testing MSE of ~12,000,000, this is a much better improvement than in model 1 and model 2. The nature of having such a high MSE is due to the fact that the average mean value of the feature that we are predicting is around 5000, so it is reasonable to have an error with a magnitude of that order. It won't be a small increment due to the nature of how big the denomination is. Since we use MSE, we should expect a exponential (square) of the average, resulting in an extremely high value which indicates that our final MSE of ~12,000,000 is not so bad actually. We also see the R2 score is ~0.83, which is improved from the other models. In summary, the results corroborate our theoretical understanding of the progression of our model selection throughout this study. Our linear regression model showed that the model was too simple and displayed a high MSE; also based on our exploratory data analysis, the graphed data did not seem to display a linear relationship. Therefore, our neural network for Model 2 improved upon this by introducing non-linearity into the predictions, allowing us to learn complex relationships and use more features in the prediction. By this reasoning, we improved the MSE drastically by over 200%. Based on these results, it was clear that Model 3 - the random forest regressor and partly the extra trees regressor - performed better than the DNN and linear regressor. They are both based upon decision trees, a model that inherently selects the most relevant features from the data set, and features that are more informative in predicting the target variable appear closer to the tree. This allowed us to automatically perform feature selection with all the features, since we found from our NN training that having more features could potentially yield better results. Moreover, random forests and extra trees regressors are ensemble methods that leverage multiple decision trees to improve predictive performance so as expected, they performed better than our linear regression and NN in particular, as it was less complex to measure feature importance during model prediction and easier to hyperparameter tune.
